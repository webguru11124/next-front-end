    ...
    return (
        <div className='mt-7'>
            <div className='grid grid-cols-5 px-2 mb-3'>
                ...
                {["Read", "Write"].map(permission => <div className='text-center' key={permission} >{permission}</div>)}
            </div>
            {Tables.slice(0, 3).concat(Tables.slice(-1)).map((table, index) => (
                <div className='bg-gray-white rounded-sm mb-5 py-4 px-2 ' key={table}>
                    <RadioGroup defaultValue={false}
                        onChange={(value) => {
                            setValue(`access.${index}.table`, table);
                            setValue(`access.${index}.access`, value);
                        }}>
                            ...
                            (< RadioGroup.Option key={`${table}-${permision} `} value={permision}>
                                {({ active, checked }) => (
                                    <div className='text-center cursor-pointer'>
                                        ...
                                        ${checked ? "bg-blue-primary" : "bg-none"}`}></span>
                                        </div>
                                    </div>
                                )}
                            </RadioGroup.Option>

...
export default function useOrganizationCreate() {
    ...
    const orgCreate = (formData: OrgForm) => axios.post("organization", { ...formData, email: "abc@abc.com" });
    const { mutate, isLoading, isError, error, data } = useMutation({
        mutationFn: orgCreate,
        onSuccess: (data, variables, context) => {
            toast.error(`Organization Created Successfully`, { hideProgressBar: true, autoClose: 5000, type: 'success', position: 'top-right' })
